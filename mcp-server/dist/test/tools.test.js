...
const mockAuth = { userId: 'u1', email: 'u@e.com', role: 'admin', token: 'test-token' };
// mock fetch globally
global.fetch = vi.fn();
beforeEach(() => {
    vi.resetAllMocks();
...
describe('convert-to-invoice', () => {
    it('converts estimate to invoice', async () => {
        ...
        const result = await convertToInvoice({ documentId: 'EST-001', dueDate: '2026-02-28' }, mockAuth);
        expect(result).toBeDefined();
    });
});
...
describe('list-documents', () => {
    ...
    it('passes filters as query params', async () => {
        ...
        const { listDocuments } = await import('../tools/list-documents.js');
        await listDocuments({ documentType: 'estimate', clientName: '株式会社テスト', limit: 10 }, mockAuth);
...
describe('update-document', () => {
    it('updates document fields', async () => {
        ...
        const result = await updateDocument({ documentId: 'DOC-001', subject: '更新後件名' }, mockAuth);
        expect(result).toBeDefined();
    });
});
...
describe('update-status', () => {
    it('updates document status', async () => {
        ...
        const result = await updateStatus({ documentId: 'DOC-001', newStatus: 'confirmed' }, mockAuth);
        expect(result).toBeDefined();
    });
});
describe('approve-document', () => {
    it('approves document', async () => {
        ...
        const result = await approveDocument({ documentId: 'DOC-001', action: 'approve', comment: '問題なし' }, mockAuth);
        expect(result).toBeDefined();
    });
    it('rejects document', async () => {
        ...
        const { approveDocument } = await import('../tools/approve-document.js');
        const result = await approveDocument({ documentId: 'DOC-001', action: 'reject', comment: '金額が間違っています' }, mockAuth);
        expect(result).toBeDefined();
    });
});
...
describe('create-client', () => {
    it('creates a new client', async () => {
        ...
        const result = await createClient({ name: '株式会社新規', email: 'contact@example.com' }, mockAuth);
        expect(result).toBeDefined();
    });
});
describe('update-client', () => {
    it('updates client info', async () => {
        ...
        const result = await updateClient({ clientId: 'CLT-001', name: '株式会社更新済み' }, mockAuth);
        expect(result).toBeDefined();
    });
});
